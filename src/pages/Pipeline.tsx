...
export default function Pipeline() {
  ...
  const getStageQuotes = (stage: string) => quotes.filter(q => q.status === stage);
  const getStageValue = (stage: string) => getStageQuotes(stage).reduce((s, q) => s + (parseFloat(String(q.total_amount)) || 0), 0);
