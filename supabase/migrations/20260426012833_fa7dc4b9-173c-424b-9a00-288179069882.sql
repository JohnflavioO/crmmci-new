...
BEGIN
  ...
  LEFT JOIN LATERAL (
    SELECT
      ...
      COUNT(*) FILTER (WHERE q.status IN ('draft', 'sent', 'pre_venda', 'pre_sale', 'contato_feito', 'contact_made', 'negociacao', 'negotiation')) AS pending_cnt,
  ...
  WHERE p.active = true
    ...
    AND p.full_name NOT ILIKE '%Francisco Flavio%'
    AND p.full_name NOT ILIKE '%Francisco Oliveira%'
    AND p.full_name NOT ILIKE '%Admin teste%'
